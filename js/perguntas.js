const perguntas = [
  {
    texto: "Quais são os domínios morfoclimáticos do Brasil?",
    opcoes: [
      "Amazônico",
      "Cerrado",
      "Caatingas",
      "Araucárias",
      "Pradarias",
      "Mares de morros"
    ],
    respostasCertas: [
      "Amazônico",
      "Cerrado",
      "Caatingas",
      "Araucárias",
      "Pradarias",
      "Mares de morros"
    ]
  },
  {
    texto: "Qual domínio é caracterizado por clima equatorial úmido e floresta densa?",
    opcoes: [
      "Amazônico",
      "Caatingas",
      "Cerrado",
      "Araucárias",
      "Pradarias",
      "Mares de morros"
    ],
    respostasCertas: ["Amazônico"]
  },
  {
    texto: "Qual domínio possui vegetação adaptada à seca e clima semiárido?",
    opcoes: [
      "Cerrado",
      "Caatingas",
      "Mares de morros",
      "Pradarias",
      "Araucárias",
      "Amazônico"
    ],
    respostasCertas: ["Caatingas"]
  },
  {
    texto: "Qual unidade paisagística é caraccterizada por planícies alagadas e rica biodiversidade aquática?",
    opcoes: [
      "Pantanal",
      "Caatingas",
      "Amazônico",
      "Pradarias",
      "Cerrado",
      "Mares de morros"
    ],
    respostasCertas: ["Pantanal"]
  },
  {
    texto: "Quais domínios apresentam vegetação predominantemente herbácea?",
    opcoes: [
      "Pradarias",
      "Araucária",
      "Mares de morros",
      "Caatingas",
      "Cerrado",
      "Amazônico"
    ],
    respostasCertas: ["Pradarias"]
  },
  {
    texto: "Qual das afirmações apresenta uma característica INCORRETA sobre o domínio morfoclimático dos Mares de Morros?",
    opcoes: [
      "Relevo mamelonar com morros arredondados",
      "Presença de solos férteis, como o massapê",
      "Clima tropical úmido com altas precipitações",
      "Vegetação original predominantemente de Mata Atlântica",
      "Ocupação humana intensa e concentrada nas áreas litorâneas",
      "Formação geológica recente, com predomínio de rochas sedimentares"
    ],
    respostasCertas: ["Formação geológica recente, com predomínio de rochas sedimentares"]
  },
  {
    texto: "O Pinheiro do Paraná é uma espécie vegetal símbolo do domínio morfoclimático:",
    opcoes: [
      "Amazônico",
      "Mares de morros",
      "Cerrado",
      "Caatingas",
      "Araucárias",
      "Pradarias"
    ],
    respostasCertas: ["Araucárias"]
  },
  {
    texto: "Sobre o domínio morfoclimático do Cerrado, podemos afirmar que:",
    opcoes: [
      "É caracterizado por um clima tropical, com alternância de verões quentes e chuvosos e invernos secos",
      "Sua vegetação é predominantemente composta por árvores de grande porte, formando uma floresta densa",
      "O solo do Cerrado é rico em nutrientes, o que favorece a agricultura sem a necessidade de adubação",
      "Expansão da agricultura e a pecuária extensiva são as principais ameaças ao Cerrado",
      "A fauna do Cerrado é rica em biodiversidade, abrigando diversas espécies endêmicas",
      "Está localizado exclusivamente na região Centro-Oeste do Brasil"
    ],
    respostasCertas: [
      "É caracterizado por um clima tropical, com alternância de verões quentes e chuvosos e invernos secos",
      "Expansão da agricultura e a pecuária extensiva são as principais ameaças ao Cerrado",
      "A fauna do Cerrado é rica em biodiversidade, abrigando diversas espécies endêmicas"]
  },
  {
    texto: " Qual domínio morfoclimático é conhecido como berço das águas?",
    opcoes: [
      "Amazônico",
      "Mares de morros",
      "Cerrado",
      "Caatingas",
      "Araucárias",
      "Pradarias"
    ],
    respostasCertas: ["Cerrado"]
  },
  {
    texto: "Quais áreas entre domínios podem ser consideradas Faixas de transição?",
    opcoes: [
      "Mata dos cocais",
      "Araucárias",
      "Agreste",
      "Complexo do Pantanal",
      "Cerrado",
      "Mata Atlântica"
    ],
    respostasCertas: ["Mata dos Cocais”, “Agreste”, “Complexo do Pantanal"]
  }
];
